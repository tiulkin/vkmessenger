 ...
 *     });
 */
Ext.define('Ext.plugin.ListPagingReverse', {
    extend: 'Ext.Component',
    alias: 'plugin.listpagingreverse',
    ...
    onScrollEnd: function(scroller, x, y) {
        ...
        if (!this.getLoading() && y <= scroller.minPosition.y) {
    ...
    updateLoading: function(isLoading) {
        ...
        if (isLoading) {
			...
			//setTimeout("VKM.app.globals['doNotScrollChat']=false;",1000);
			var list = this.getList();
			if(list)
			{
				list.scrollToRecord(VKM.app.globals['scrollToRecord'])
				setTimeout("Ext.ComponentQuery.query('#chatList')[0].doNotScroll=false;",500)
				//list.doNotScroll='ok';
			}
    ...
    loadNextPage: function() {
		...
		var list = this.getList();
		VKM.app.globals['scrollToRecord'] = list.getItemAt(0).getRecord();		
